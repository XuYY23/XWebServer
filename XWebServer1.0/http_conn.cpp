...
void http_conn::init(int sockfd, const sockaddr_in& addr)
{
   m_sockfd = sockfd;
   m_address = addr;
   /*以下两行是为了避免TIME_WAIT状态，仅用于测试，实际使用时应该去掉*/
   int reuse= -1;
...
http_conn::parse_request_line(char* text)
{
   ...
   *m_url++ = '\0'; //将分隔符变为结束符，可以让后续的method直接等于text来获取请求方法
   char* method = text;
   ...
   m_url += strspn(m_url, " \t");//忽略开头的空白符
   m_version = strpbrk(m_url, " \t");//和获取请求方法同理，从该指针所指位置开始剩下的就是版本号
   ...
   *m_version++ = '\0';//将url后面的空格符变成结束符，这样m_url就是一个单独的字符串
...
http_conn::parse_headers(char* text)
{
   ...
   if(text[0] == '\0')
   {
      /*如果HTTP请求有消息体，则还需要读取m_count_length字节的消息体，状态机转移到CHECK_STATE_CONTENT状态*/
   ...
   else if(strncasecmp(text, "Connection:", 11) == 0)
   {
      text += 11;
      text += strspn(text, " \t");
      if(strcasecmp(text, "keep-alive") == 0) //判断是否保持长连接
      {
         m_linger = true;
      }
   }
...
bool http_conn::write()
{
   ...
   while(true)
   {
      ...
      {
         /*发生 HTTP 响应成功，根据 HTTP 请求中的 Connection 字段决定是否立即关闭连接*/
         unmap();
         if( m_linger )
         {
            init();
...
/*由线程池中的工作线程调用，这里是处理 HTTP 请求的入口函数*/
